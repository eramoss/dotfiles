...
Singleton {
	...
	function getData() {
		...
		// fixed city name
		command +=
		`jq --arg city "$(curl -s 'https://nominatim.openstreetmap.org/reverse?format=json&accept-language=en&lat=${root.location.lat}&lon=${root.location.long}' | jq -r '.address.city // .address.town // .address.village // .address.municipality // "Unknown"')" ` +
		"'{current: .current_condition[0], location: { areaName: [{ value: $city }] }, astronomy: .weather[0].astronomy[0]}'";
